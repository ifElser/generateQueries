...
function generateQueries(keywords, maxCharLength, delimiter){
	...
	while(keywords.length && keywords[0].length >= maxCharLength - delimiter.length) queries.push(keywords.shift());
	while(keywords.length){
		let index = 0;
		query = query || keywords.shift();
		while(index < keywords.length && query.length + delimiter.length + keywords[index].length > maxCharLength) index++;
		if(index === keywords.length) {
			queries.push(query);
			query = '';
		} else query = keywords.splice(index,1).concat(query).join(delimiter);
	}
